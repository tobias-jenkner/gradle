...
* Since constants can be inlined, any change to a constant will result in Gradle recompiling all source files. For that reason, you should try to minimize the use of constants in your source code and replace then with static methods where possible, so that you can enjoy the full benefits of incremental compilation.
...
* Gradle will even incrementally analyze JAR contents, so the incremental compiler works across project boundaries.
* A loosely coupled code base will have shorter dependency chains and thus compile faster on incremental changes.
...
You can then register it for incremental compilation using the `getSupportedOptions()` method.
...
<sample xmlns:xi="http://www.w3.org/2001/XInclude" id="incrementalAnnotationProcessorRegistration" dir="java/incrementalAnnotationProcessing" title="Registering incremental annotation processors">
     <sourcefile file="processor/src/main/java/EntityProcessor.java" snippet="registering-incremental-annotation-processors"/>
</sample>
++++

Both categories have the following limitations:

* They must not have any side effects except for using the `Filer` and `Messager` APIs.
    For instance, writing files using `java.io` will result in silent failures later on, as these files won't be cleaned up correctly.
    If your processor does this, it cannot be incremental.
* They must not depend on compiler-specific APIs like `com.sun.source.util.Trees`. Gradle wraps the processing APIs, so attempts to cast to compiler-specific types will fail.
    If your processor does this, it cannot be incremental, unless you have some fallback mechanism.
* If they use `Filer#getResource` Gradle will recompile all source files. See https://github.com/gradle/gradle/issues/4701[gradle/issues/4701]
* If they use `Filer#createResource` Gradle will recompile all source files. See https://github.com/gradle/gradle/issues/4702[gradle/issues/4702]

====== Isolating annotation processors
The most efficient category, these have to look at each annotated element in isolation, creating generated files or validation messages for it.
